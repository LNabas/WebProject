...
class Auth
{
    ...
    public function register($db, $username, $email, $password, $promotion){
        $password = $this->hashPassword($password);
        $token = Str::random(60);
        $db->query("INSERT INTO users SET username = ?, email = ?, password = ?, promotion = ?, confirmation_token = ?",[
            $username,
            $email,
            $password,
            $promotion,
        ...
        mail($email, 'confirmation de votre compte', "Afin de valider votre compte , merci de cliquer sur ce lien \n\nhttp://localhost/PROJET_WEB/confirm.php?id=$user_id&token=$token");
    }
    ...
    public function resetPassword($db, $email){
        ...
        if($user){
            ...
            mail($_POST['email'], 'Réinitialisation de votre mot de passe', "Afin de Réinitialisation votre mot de passe , merci de cliquer sur ce lien \n\nhttp://localhost/PROJET_WEB/reset.php?id={$user->id}&token=$reset_token");
            return $user;
        }
        return false;
    }
    ...
    public function checkResetToken($db, $user_id, $token){
        return $db->query('SELECT * FROM users WHERE id = ? AND reset_token IS NOT NULL AND reset_token = ? AND reset_at > DATE_SUB(NOW(), INTERVAL 60 MINUTE)' ,[$user_id, $token])->fetch();
    }
